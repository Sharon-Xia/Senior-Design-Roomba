...
class Safety(object):
    ...
    """
    safety_threshold = .7 # ttc threshold

    scan_sub_rate = 100 # once every 50 msgs
    scan_sub_tick = 0
    ...
    # only check for collisions if angle is within certain scope
    def is_in_scope(self, theta):
        return (theta < (pi/4) and theta > (-pi/4)) or (theta > (pi * 3/4) or theta < (pi * -3/4))
    ...
    def scan_callback(self, scan_msg):
        ...
        if self.scan_sub_tick >= self.scan_sub_rate:
            ...
            for i, d in enumerate(scan_msg.ranges): # index, distance
                ...
                if (rd > 0 and self.is_in_scope(theta)):
                    ttc = d / (rd)
                    if (ttc < TTC):
                        TTC = ttc # ttc's
                        ttc_theta = theta
